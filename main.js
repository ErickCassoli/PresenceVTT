...
if (portableDir) {
  app.setPath('userData', path.join(portableDir, 'presencevtt-data'));
}
...
ipcMain.handle('create-backup-zip', async (event, destPath, scenesData, extras) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    // Profile extras (optional): soundboard/systems JSON + sound files → makes this a full "profile" zip.
    if (extras) {
      if (extras.profileJson) archive.append(String(extras.profileJson), { name: 'profile.json' });
      if (Array.isArray(extras.files)) {
        for (const f of extras.files) { if (f && f.name && f.buffer) archive.append(Buffer.from(f.buffer), { name: f.name }); }
      }
    }
...
// Reads profile.json (as string) and every sounds/* entry (as ArrayBuffer) from a profile zip.
ipcMain.handle('read-backup-profile', async (_event, zipPath) => {
  return new Promise((resolve, reject) => {
    yauzl.open(zipPath, { lazyEntries: true }, (err, zipfile) => {
      if (err) return reject(err);
      ...
      zipfile.readEntry();
      zipfile.on('entry', entry => {
        if (entry.fileName === 'profile.json') {
          zipfile.openReadStream(entry, (e2, rs) => {
            if (e2) { zipfile.readEntry(); return; }
            const c = []; rs.on('data', d => c.push(d));
            rs.on('end', () => { profileJson = Buffer.concat(c).toString('utf8'); zipfile.readEntry(); });
            rs.on('error', () => zipfile.readEntry());
          });
        } else if (/^sounds\/[^/]+$/.test(entry.fileName)) {
          zipfile.openReadStream(entry, (e2, rs) => {
            if (e2) { zipfile.readEntry(); return; }
            const c = []; rs.on('data', d => c.push(d));
            rs.on('end', () => { const buf = Buffer.concat(c); sounds.push({ name: entry.fileName, buffer: buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength) }); zipfile.readEntry(); });
            rs.on('error', () => zipfile.readEntry());
          });
        } else { zipfile.readEntry(); }
      });
      zipfile.on('end', () => { zipfile.close(); resolve({ profileJson, sounds }); });
      zipfile.on('error', reject);
    });
  });
});
